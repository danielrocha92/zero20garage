...
export default function CrmCampanhas() {
  ...
  return <main className={styles.page}><header className={styles.header}><p className={styles.eyebrow}>PÓS-VENDA E PREVENÇÃO</p><h1>CRM de Campanhas</h1><p>Selecione clientes elegíveis, personalize a mensagem e dispare a campanha pelo canal integrado.</p></header><div className={styles.layout}><FiltrosCRM value={filters} onChange={setFilters} /><EditorCampanha templates={templates} mensagem={message} onMensagemChange={setMessage} onSelecionarTemplate={(template) => { setCampaignName(template.nome); setMessage(template.mensagem); }} onSalvarTemplate={async (nome, mensagemValue) => { const template = await salvarTemplateCRM(nome, mensagemValue); setTemplates((current) => [...current, template]); setCampaignName(nome); }} /><div className={styles.actions}><span>{selected.length} selecionado(s) de {filtered.length}</span><button type="button" className={styles.dispatch} onClick={send} disabled={sending || !selected.length || !message.trim()}>{sending ? 'Disparando...' : 'Disparar Campanha'}</button></div>{feedback.error && <p className={styles.error} role="alert">{feedback.error}</p>}{feedback.success && <p className={styles.success} role="status">{feedback.success}</p>}{loading ? <p className={styles.feedback}>Carregando clientes...</p> : <TabelaClientesCRM clientes={filtered} selecionados={selected} onToggle={toggle} onToggleTodos={() => setSelected((current) => filtered.every((client) => current.includes(client.id)) ? current.filter((id) => !filtered.some((client) => client.id === id)) : Array.from(new Set([...current, ...filtered.map((client) => client.id)])))} />}</div></main>;
}
