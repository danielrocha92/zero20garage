...
export default function TabelaClientesCRM({ clientes, selecionados, onToggle, onToggleTodos }: Props) {
  ...
  return <div className={styles.wrapper}><table><caption className={styles.caption}>Clientes elegíveis para campanha</caption><thead><tr><th><input type="checkbox" checked={todosSelecionados} onChange={onToggleTodos} aria-label="Selecionar todos" /></th><th>Cliente</th><th>Veículo</th><th>Telefone</th><th>Status da Revisão</th><th>Último Contato</th></tr></thead><tbody>{clientes.map((cliente) => <tr key={cliente.id}><td><input type="checkbox" checked={selecionados.includes(cliente.id)} onChange={() => onToggle(cliente.id)} aria-label={`Selecionar ${cliente.cliente}`} /></td><td>{cliente.cliente}</td><td>{cliente.veiculo}</td><td>{cliente.telefone || 'Não informado'}</td><td><span className={cliente.statusRevisao.includes('Atrasada') ? styles.late : styles.pending}>{cliente.statusRevisao}</span></td><td>{cliente.ultimoContato || 'Nunca'}</td></tr>)}</tbody></table>{clientes.length === 0 && <p className={styles.empty}>Nenhum cliente encontrado com os filtros atuais.</p>}</div>;
}
