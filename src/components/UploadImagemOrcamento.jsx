...
const UploadImagemOrcamento = ({ orcamentoId, authToken, imagemAtual = [], onUploaded }) => {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      files.forEach(f => form.append('imagem', f));
      ...
      try {
        ...
        alert('Falha no upload: resposta inválida do servidor. Veja console.');
        return;
      }
      ...
      if (res.ok) {
        ...
        alert(`Erro no upload: ${data?.msg || data?.error || 'Erro desconhecido'}`);
      }
    } catch (err) {
      console.error('Falha no upload:', err);
      alert('Falha no upload de imagens. Veja console.');
    } finally {
      setUploading(false);
    }
  };
