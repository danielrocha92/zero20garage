...
  // Ajuste na função para retornar string vazia se o valor for 0 ou NaN.
...
      const pdfWidth = 210;
      // Calcula a altura do PDF com base no aspecto do canvas
      const pdfHeight = (canvas.height * pdfWidth) / canvas.width;
      const pdf = new jsPDF('portrait', 'mm', [pdfWidth, pdfHeight]);

      // Adiciona a primeira página (o conteúdo renderizado do HTML)
      pdf.addImage(imgData, 'PNG', 0, 0, pdfWidth, pdfHeight);

      // Adiciona as imagens originais em páginas separadas
...
    // Caso 1: Timestamp do Firebase/Firestore
    if (typeof orcamento.data === 'object' && orcamento.data._seconds) {
      dateToFormat = dayjs.unix(orcamento.data._seconds).local();
    // Caso 2: String de data (ex: YYYY-MM-DD) ou Objeto Date nativo
    } else {
      dateToFormat = dayjs(orcamento.data);
    }

    if (dateToFormat.isValid()) formattedDate = dateToFormat.format('DD/MM/YYYY');
  }
...
      <div id="orcamento-main-content" className="orcamento-impresso-content" ref={componentRef}>
        {/* Cabeçalho */}
        <div id="orcamento-header" className="header-impresso">
          <h1>ORÇAMENTO - {orcamento.tipo === 'motor' ? 'MOTOR COMPLETO/PARCIAL' : 'CABEÇOTE'}</h1>
          <img src={logo} alt="Logo Zero20Garage" className="logo-impresso" id="logo-impresso" />
        </div>
...
          <p className="payment-method"><strong>Forma de Pagamento:</strong> {orcamento.formaPagamento || ''}</p>
